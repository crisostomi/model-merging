"""
Multi-task supervised fine-tuning with BALANCED (task-adaptive) sample counts.

Fixes the SUN397 regression from grow_finetune_supervised.py:
- The uniform 500 samples/task gave SUN397 (397 classes) only ~1.3 samples/class
- This version scales samples by number of classes:
    <= 20 classes:  500 samples (~25+ samples/class)
    20-100 classes: 1000 samples (~20+ samples/class)
    > 100 classes:  2000 samples (~5-10 samples/class)
- Also increases epochs from 20 -> 30 for better convergence on harder tasks.

Total training budget: ~8000 samples (vs 4000 uniform), still very manageable.
...
5. Load training data: task-adaptive sample counts with labels
...
Usage:
    sbatch slurm/launch_analysis.slurm scripts/grow_finetune_balanced.py
    sbatch slurm/launch_analysis.slurm scripts/grow_finetune_balanced.py --quick
"""
...
# Task-adaptive sample counts: scale by number of classes
# SUN397=397 classes, Cars=196 -> 2000 samples (~5-10 samples/class)
# RESISC45=45, DTD=47          -> 1000 samples (~20 samples/class)
# SVHN=10, GTSRB=43, MNIST=10, EuroSAT=10 -> 500 samples (~50 samples/class)
SAMPLES_PER_TASK = {
    "SUN397": 2000,
    "Cars": 2000,
    "RESISC45": 1000,
...
# --------------------------------------------------------------------------- #
# Multi-task data loading (with labels) -- BALANCED version
# --------------------------------------------------------------------------- #
...
def load_task_datasets(preprocess, datasets, samples_per_task, batch_size=32):
    """Load train and test data for all tasks with task-adaptive sample counts.

    Args:
        preprocess: Image preprocessing transform
        datasets: List of dataset names
        samples_per_task: Dict mapping dataset name -> number of training samples
        batch_size: Batch size for data loaders

    Returns:
        train_loaders: dict of {task_name: DataLoader} for training
    ...
    for ds_name in datasets:
        ...
        n_train_for_task = samples_per_task[ds_name]
        logger.info(f"  Loading {ds_name} (target: {n_train_for_task} samples)...")
        ...
        n_use = min(n_train_for_task, n_avail)
        if n_use < n_avail:
    ...
    logger.info(f"  Total training samples across all tasks: {total_train}")
...
def train_multitask(encoder, classification_heads, train_loaders, device,
    ...
    logger.info(f"  Samples per task: {dict((t, len(train_loaders[t].dataset)) for t in task_names)}")
...
def plot_comparison(baseline, expanded, output_path):
    ...
    bars2 = ax.bar(x + width/2, exp_vals, width, label="Balanced expand + supervised", color="#DD8452", alpha=0.85)

    ax.set_xlabel("Task")
    ax.set_ylabel("Accuracy")
    ax.set_title("Balanced Multi-task Supervised Fine-tuning (task-adaptive samples)")
    ax.set_xticks(x)
    ...
    ax.axhline(y=exp_avg, color="#DD8452", linestyle="--", alpha=0.5, label=f"Balanced avg: {exp_avg:.4f}")
...
def main():
    parser = argparse.ArgumentParser(description="Balanced supervised fine-tuning of expanded merged model")
    ...
    if args.quick:
        args.epochs = 3
        # Scale down all sample counts proportionally
        samples_per_task = {k: max(v // 5, 50) for k, v in SAMPLES_PER_TASK.items()}
    else:
    ...
    # ------------------------------------------------------------------- #
    logger.info("=" * 60)
    logger.info("STEP 3: Load multi-task training and test data (BALANCED)")
    logger.info("=" * 60)
    ...
    # ------------------------------------------------------------------- #
    logger.info("=" * 60)
    logger.info("STEP 6: Multi-task supervised fine-tuning (BALANCED)")
    logger.info("=" * 60)
    ...
    all_results = {
        "config": {
            "method": "balanced_supervised_multitask_classification",
            ...
            "batch_size": args.batch_size,
            "samples_per_task": samples_per_task,
            "total_training_samples": sum(samples_per_task.values()),
    ...
    json_path = OUTPUT_DIR / "grow_finetune_balanced_results.json"
    with open(json_path, "w") as f:
        json.dump(all_results, f, indent=2)
    logger.info(f"  Results saved to: {json_path}")

    # Plot comparison
    plot_path = OUTPUT_DIR / "balanced_comparison.png"
